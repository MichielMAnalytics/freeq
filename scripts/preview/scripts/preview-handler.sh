...
BOOT_COMMENT_ID=$(post_comment "<img src=\"https://raw.githubusercontent.com/MichielMAnalytics/boxd-assets/main/boxd-burst-light.gif\" width=\"32\" align=\"absmiddle\"/> creating boxd preview env for \`$PREVIEW_BRANCH\` → $URL
...
if [ -n "$EXISTING_STATUS" ] && [ "$EXISTING_STATUS" != "failed" ]; then
  ...
  for attempt in $(seq 1 $MAX_FORK_ATTEMPTS); do
    ...
    if [ "$attempt" -lt "$MAX_FORK_ATTEMPTS" ]; then
      ...
      update_boot_progress "<img src=\"https://raw.githubusercontent.com/MichielMAnalytics/boxd-assets/main/boxd-burst-light.gif\" width=\"32\" align=\"absmiddle\"/> creating boxd preview env for \`$PREVIEW_BRANCH\` → $URL
